...
/**
 * A DataFormula describes a (possibly computed) attribute of a
 * ResourceContext.  DataFormulas are gotten from the corresponding
 * context, by name.  In general the name will be a toplevel static
 * inner class of the context (which implies only one formula per
 * type). */
abstract public class DataFormula 
    implements DataValueChangedCallbackListener, Constants
{
    private String name;
    private String[] args;
    ...
    /**
     * Instantiable subclasses provide this to do the formula's work,
	...
	synchronized (dependencies) {
	    if (deleted) return DataValue.NO_VALUE;
	...
	if (args == null) return this.args == null || this.args.length == 0;
	if (this.args == null) return args.length == 0;
    ...
    protected DataFormula registerDependency(ResourceContext context,
					     ...
					     String key)
    {
	DataFormula requiredFormula = 
	    context.getFormula(formulaName, formulaArgs);
	if (requiredFormula != null) {
	    registerDependency(requiredFormula, key);
	} else {
	    Logger logger = Logging.getLogger(DataFormula.class);
	    logger.error("### Failed to find formula " +
			       formulaName + " in context " +
			       context);
	}
    ...
    /**
     * Returns a list of other formulas on which this one depends.
     * Only used by the ResourceContext gui (so far). */
    ...
    /** 
     * Implements the DataValueChangedCallbackListener.  This would
     * typically be called here when one of the other formulas on
     * which this one depends has a new value. */
    ...
    /**
     * Returns the currently cached value.  This is part of the
     * standard (non-gui) DataFormula interface. */
    public DataValue query() {
	return cachedValue;
    }

    /**
     * Computes and returns a new value.  This is part of the standard
    ...
    /**
     *  Registers the listener so that will be called back when the
     *  value of this one changes.  This is part of standard (non-gui)
	...
	synchronized (listeners) {
		...
		Logger logger = Logging.getLogger(DataFormula.class);
		logger.warn(listener + " subscribed to deleted formula " +
			    this);
		return;
    ...
    /**
     *  Unregisters the listener for callbackes. This is part of the
     *  standard (non-gui) DataFormula interface. */
