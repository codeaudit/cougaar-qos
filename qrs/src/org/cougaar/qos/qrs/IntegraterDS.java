 ...
 * selects the 'best' value by maximizing validity.  The only
 * available formula is 'Formula'. */
public class IntegraterDS extends ResourceContext
{
    ...
    protected IntegraterDS(String[] keys, ResourceContext parent) 
    ...
    /**
     * The parameters should contain one object, a String, which will
     * be the key used to register this context as a listener on the
     * various DataFeeds. */
	...
	extends DataFormula implements DataFeedListener
    {
	...
	private void rescanFeeds(DataValue old) {
	    ...
	    if (best != old) setCachedValue(best);
	}


	/**
	 * When new data arrives from any feed, the cached 'best'
	 * value will be updated.  If the new data comes from the same
	 * feed that previously supplied the 'best' value, and if the
	 * validity of the new data is greater than or equal to the
	 * validity the previous best, the new data becomes the best
	 * data.  If the new data comes from the same feed that
	 * previously supplied the 'best' value, and if the validity
	 * of the new data is less than the validity the previous
	 * best, the feeds are scanned for a new best.  If the new
	 * data comes from the different feed than the one that
	 * previously supplied the 'best' value, and if the validity
	 * of the new data is higher than that of the previos best,
	 * the new data becomes the best data.  Otherwise the previous
	 * stays best. */
	public synchronized void newData(DataFeed store, 
					 String key,
					 DataValue data)
	{
	    if (logger.isDebugEnabled())
		logger.debug("Integrater for " +key+
			     " got new data " +data+
			     " from " +store.getName());
