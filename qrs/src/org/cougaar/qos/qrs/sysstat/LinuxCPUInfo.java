    ...
    extends SysStatHandler
{
	...
	String prefix = "Host" + KEY_SEPR + host + 
	    KEY_SEPR + "CPU" + KEY_SEPR;
	...
	try {
	    return new DataValue(parseValue(doub),
	...
	try {
	    while(Character.isDigit(line.charAt(end_index))) ++end_index;
	    double cache = 
		Double.parseDouble(line.substring(start_index, end_index));
	    
	    return new DataValue(cache,
				 SECOND_MEAS_CREDIBILITY,
				 "",
				 PROVENANCE);
	} catch (NumberFormatException nf_ex) {
