    ...
    extends SysStatHandler
{
    ...
    {
	free_key = "Host" + KEY_SEPR + 
	    host + KEY_SEPR + 
	    "Memory"+  KEY_SEPR + 
	    "Physical" + KEY_SEPR + 
	    "Free";
	total_key = "Host" + KEY_SEPR + 
	    host + KEY_SEPR + 
	    "Memory"+  KEY_SEPR + 
	    "Physical" + KEY_SEPR + 
	    "Total";
	util_key = "Host" + KEY_SEPR + 
	    host + KEY_SEPR + 
	    "Memory"+  KEY_SEPR + 
	    "Physical" + KEY_SEPR + 
	    "Utilization";
    ...
    {
	int start = 8;
	while (!Character.isDigit(line.charAt(start))) ++start;
	int end = start;
	while (Character.isDigit(line.charAt(end))) ++end;
	...
	if (free_dv != null) map.put(free_key, free_dv);
	if (total_dv != null) map.put(total_key, total_dv);
	if (free_dv != null && 	 total_dv != null) {
	    double util = (total-free)/total;
	    DataValue util_dv =  new DataValue(util,
					       SECOND_MEAS_CREDIBILITY,
					       "",
					       PROVENANCE);
