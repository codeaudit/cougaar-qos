...
/**
 * A simple struct-like class used to represent the values manipulated
 * by the various DataScope/DataFormula objects. */
public class DataValue implements Constants, java.io.Serializable
{
    /**
     * Standard instance of a value without any specific credibility.
     * This may move into the Constants interface later. */
	...
	this(value.value, value.credibility, value.units, value.provenance,
	     value.timestamp, value.halflife);
    }
    ...
    public DataValue(Object value, 
		     ...
		     String provenance) 
    {
	this(value, credibility, units, provenance,
	...
	this((Object) value, credibility, null, null);
    }

    public DataValue(String value) {
	this((Object) value, DEFAULT_CREDIBILITY, null, null);
    }
	...
	return new DataValue(raw, credibility, units, provenance,
			     timestamp, halflife);
    }
	
    public DataValue newCredibility(double new_credibility) {
	return new DataValue(value, new_credibility, units, provenance,
			     timestamp, halflife);
    }
	...
	return new org.cougaar.qos.ResourceStatus.DataValue(timestamp, halflife,
					       credibility, 
					       (units != null ? units : ""), 
	...
	return ((value == null && candidate.value == null) || 
	    ...
	    ((units == null && candidate.units == null) ||
	     (units != null && units.equals(candidate.units))) &&
	    ((provenance == null && candidate.provenance == null) ||
	     (provenance != null && 
    ...
    /**
     * Returns the value with the hightest credibility.  If the max
     * credibilitry is common to several values, returns the last one
     * (order is significant). */
    public static DataValue maxCredibility( DataValue[] values)
    {
	DataValue max = null;
	DataValue value;
	for (int i=0; i<values.length; i++) max = mostCredible(max, values[i]);
	return max;
    }
