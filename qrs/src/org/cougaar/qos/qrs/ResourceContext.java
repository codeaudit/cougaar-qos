...
/**
 * Abstract definition of a contect for resource data, ie a collection
 * of attributes bound over a set of parameter values.  The attributes
 * are implemented as DataFormula instances. */
abstract public class ResourceContext 
    implements Constants
{
    /**
     * Exception class used to indicate the one of the supplied
     * parameters is missing, has the wrong type or has an
     * unacceptable value. */
    ...
    public static void registerContextInstantiater(String kind,
						   ContextInstantiater instantiater)
    {
    ...
    /**
     * Standard constructor.  It verifies the parameters and adds the
     * context to the list of known contexts. */
    protected ResourceContext(String[] parameters, ResourceContext parent)
	...
	if (symbol_table.containsKey(symbol))
	    ...
	    logger.error("### Attempt to set value of symbol " +
			       symbol + " which has no binding");
	}
    }
    ...
    protected void invoke(String method, String[] parameters)
    ...
    /**
     * Subclasses should provide this method.  It should verify that
     * the given parameters are correct in number, position, type and
     * value, throwing ParameterError if not. */
    abstract protected void verifyParameters(String[] parameters) 
	throws ParameterError;
...
//     }
     ...
     * instantiated so far.  This is only used by the ResourceContext
     * gui. */
    ...
    /**
     * Returns a list of the ResourceContexts for a given class that have
     * been instantiated so far.  This is only used by the ResourceContext
     * gui. */
    ...
    ResourceContext resolveSpec(Object id, 
				String kind, 
				String[] parameters) 
    {
	synchronized (children) {
    ...
    protected Object nextLookup(String symbol, 
				String[] args, 
				boolean resolve) 
    {
	if (parent != null)
    ...
    protected Object lookupSymbol(String symbol, 
				  String[] args, 
				  boolean resolve) 
    {
	Logger logger = Logging.getLogger(ResourceContext.class);
...
// 		logger.info(this +" got "+ value +" for " +symbol);
    ...
    /**
     * Returns an list of Formula classes instantiated so far by this
     * context.  Only used by the ResourceContext gui. */
    ...
    /**
     * Returns a list of the Formulas for a given class that have
     * been instantiated so far.  This is only used by the ResourceContext
     * gui. */
    ...
    /**
     * Returns a list of the parameter bindings.  So far this
     * is only used by the ResourceContext gui. */
    ...
    private DataFormula makeFormula(String formula_name,
				    String[] formula_args)
    {
    ...
    private void initializeFormula(DataFormula formula, 
				   String name,
				   String[] args)
    {
    ...
    /**
     * Returns a formula from the cache for the given name, or makes
     * one (and caches it) if none is cached.  In the second case the
     * 'makeFormula()' method is used to create a new DataFormula. */
	...
	synchronized (formulas) {
		...
		for (int i=0; i<length; i++) {
		    ...
		    if (candidate.hasArgs(args)) return candidate;
		}
	    }
	    
	    // No extant formula
	    formula = makeFormula(name, args);
	    if (formula != null) forms.add(formula);

	    return formula;
	}
    }

    private DataFormula makeProxyFormula(DataFormula formula, 
					 String name,
					 String[] args)
    {
	Logger logger = Logging.getLogger(ResourceContext.class);
	if (formula == null) {
	    ...
	    synchronized (formulas) {
		if (logger.isDebugEnabled())
		    logger.debug("Making proxy for " +formula+
				 " in context " + this);
	...
	synchronized (children) {
		...
		if (kids != null) {
			...
			ResourceContext child_ctxt = (ResourceContext) 
			    child.getValue();
			ctxt_itr.remove();
	...
	synchronized (formulas) {
		...
		if (logger.isDebugEnabled()) {
		    ...
		    logger.debug("Walking through " +key+ 
				 " formulas for deletion");
		}
