...
/**
 * A sample ResourceContext which looks for ip-flow capacity data on
 * any feed, by using a Remos-style key with an IntegraterDS.  The
 * available formulas are 'CapacityMax' and 'CapacityUnused'. */
public class IpFlowDS extends ResourceContext 
{
	...
	ContextInstantiater cinst = new AbstractContextInstantiater() {
		...
		public Object identifyParameters(String[] parameters) 
		{
		    ...
		    String y = (String) parameters[1];
		    String result;
		    try {
			String a1 = InetAddress.getByName((String) x).getHostAddress();
			String a2 = InetAddress.getByName((String) y).getHostAddress();
			result = a1 +"=>"+ a2;
    ...
    /**
     * The parameters should contain two strings, the source and
...
// 	parameters[1] = dst;
    ...
    private IpFlowDS(String[] parameters, ResourceContext parent) 
	throws ParameterError
    {
	...
	protected void initialize(ResourceContext context) {
	    ...
	    String key = "Ip" + KEY_SEPR + "Flow" + KEY_SEPR +
		source + KEY_SEPR + 
		destination + KEY_SEPR + 
		getKey();
	    String[] parameters = { key };
	    ResourceContext dependency =
		RSS.instance().resolveSpec("Integrater", parameters);
	    ...
	    if (from_site == null) {
		logger.error("IP FLOW No from site matching " + 
				   source);
		return;
	    }

	    if (to_site == null) {
		logger.error("IPFLOW No to site matching " + 
				   destination);
		return;
	    }

	    String[] parameters2 = {  from_site.toString(), 
				      to_site.toString() };
	    ResourceContext dependency2 = 
		RSS.instance().resolveSpec("SiteFlow", parameters2);
	...
	protected DataValue doCalculation(DataFormula.Values values) {
		...
		logger.debug("Recalculating " +getKey()+
				   " values=" +this.values[2]+ " (ipflow) " +
				   ", " +this.values[1]+ " (siteflow) " +
