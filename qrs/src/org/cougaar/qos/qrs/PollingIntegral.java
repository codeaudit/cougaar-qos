...
/**
 * This a 'rate-ifier' abstraction.  It's designed to accumulate
 * calculated instantanteous data so that a periodic poller can supply
 * a rate to the client.  The period is supplied as an argument.  For
 * now the assumption is that raw values are doubles.  Instantiate
 * subclasses must provide two methods: configureDependencies(), to
 * set up any dependencies on other formulas, and
 * computeValueFromDependencies, which should calculate a new value
 * given a collection of dependency values.
 *
 * A useful extension is SingleKeyPollingIntegral.java, a
 * PollingIntegral which relies on a single DataFeed value.
 */
abstract class PollingIntegral extends DataFormula
{
	...
	ResourceContext dependency = 
	    RSS.instance().resolveSpec("Alarm", parameters);
	alarm = registerDependency(dependency, "AlarmFormula", 
				   getArgs(), "Alarm");
	...
	synchronized (this) {
		...
		if (logger.isDebugEnabled())
		    logger.debug("pollUpdate:" +
				" last=" +lastRunningIntegral+
				" running=" +runningIntegral+
				" avg= " +avg);
		lastRunningIntegral = runningIntegral;
