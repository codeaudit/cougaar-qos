...
public class CorbaUtils
{
    ...
    public static void writeIOR(String iorfile, 
	...
	try {
	    ...
	    Logger logger = Logging.getLogger(CorbaUtils.class);
	    logger.warn("Unable to resolve NameService ORBInitRef:"
			+ex.getMessage());
	}
    }
	...
	while (i<args.length) {
		...
		config = args[i++];
	    } else System.out.println("Do not understand switch " + arg);
	}
	...
	Logger logger = Logging.getLogger(CorbaUtils.class);
        if (logger.isDebugEnabled())
            logger.debug("Arg Switches "+
                          " -logging.props=" +logging_props_file+
                          " -gui=" +gui+
                          " -rss.config=" +config+
                          " -ior=" +iorfile );
	...
	if (config != null) {
		...
		try {
		    ...
		    logger.error("Error loading RSS properties from "
					+config+ ": " + ex);
		}
	    }
	}
	...
	if (impl != null) {
	    ...
	    if (iorfile != null) writeIOR(iorfile, reference);

	    impl.postInit();

	    System.out.println("RSS ready");
	    if (iorfile != null) System.out.println("IOR in " + iorfile);
	    orb.run();
	}
    }
