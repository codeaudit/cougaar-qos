...
public class BoundDataFormula
    extends Observable
    implements DataValueChangedCallbackListener
{
    ...
    /**
     *  Make a forward-chained binding of a context and a symbol in
     *  that context.  If keepCurrent is true, a thread will be
     *  created to retrieve the value periodically.  If keepCurrent is
     *  false, the value will only be retrieved on request.  */
    public BoundDataFormula(ResourceContext context, 
			    String symbol,
			    boolean keepCurrent) 
    {
	this.symbol = symbol;
	...
	if (keepCurrent) {
	    update();
	    poller = new Runnable() { public void run() { update(); }};
	    RSSUtils.schedule(poller, 0, 1000);		    
	}
    }

    /**
     * Make a backward-chained binding of a formula.  It will be
     * notified when the formula's value changes.  */
    public BoundDataFormula(DataFormula formula) 
	throws NullFormulaException
    {
	if (formula == null) throw new NullFormulaException();
	...
	if (!delayed) {
	    ...
	    if (f == null) throw new NullFormulaException();
	    setFormula(f);
	} else {
	...
	Logger logger = Logging.getLogger(BoundDataFormula.class);
	    logger.warn("Replaced deleted formula " +formula+
			 " with " +replacement);
    ...
    /**
     * Callback from the DataFormula indicating that the value has
     * changed.  This implements the DataValueChangedCallbackListener
     * interface. */
