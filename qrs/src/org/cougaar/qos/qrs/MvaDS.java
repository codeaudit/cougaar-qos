...
/**
 * Mean Value Analysis model for Latency
 * This assumes that component latencies are separable.
 */
public class MvaDS extends ResourceContext implements Constants
{
    ...
    /**
     * The parameters should contain one string, the type of model, in
     * this case MVA */
	...
	if (parameters == null || parameters.length != 1) {
	    ...
	    String modelType = (String) parameters[0];
	    bindSymbolValue(MODELTYPE, modelType);

	}
    }

    private MvaDS(String[] parameters, ResourceContext parent) 
	throws ParameterError
    {
	...
	protected DataValue doCalculation(DataFormula.Values values) {
	    ...
	    double latency =
		 (( replySize  * replyIpb)         // reply instructions 
		    + (requestSize * requestIpb))  // requestinstructions 
