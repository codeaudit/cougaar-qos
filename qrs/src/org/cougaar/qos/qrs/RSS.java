...
public final class RSS extends ResourceContext
{
	...
	if (is != null) {
		...
		static_logger.error("Error loading RSS properties from "
				   +name+ ": " + ex);
	    }
	}
    ...
    private Hashtable data_feeds;
    private SitesDB sites;
    private String sitesurl_string;
    private Properties props;
    private Object sites_lock = new Object();
    private Set[] event_subscribers = new Set[EVENT_COUNT];
    ...
    /**
     * This constructor will try to instantiate the feeds listed in
     * the given Properties collection. */
    private RSS(Properties props) 
	...
	if (raw_feed_data != null) {
		...
		new StringTokenizer(raw_feed_data, " ");
	...
	if (sysstat_interval_str != null) {
		...
		new  DirectSysStatSupplier(null,feed);
	    supplier.schedule(sysstat_interval);
	}

	String gui_id = props.getProperty(GUI_PROPERTY);
	if (gui_id != null) new org.cougaar.qos.qrs.gui.MainWindow(gui_id);
    ...
    private HashMap global_cache = new HashMap();

    public void deleteContext(ResourceContext context) {
	synchronized (global_cache) {
	    HashMap classcache = (HashMap) 
		global_cache.get(context.getContextKind());
	    if (classcache != null) classcache.remove(context.getID());
	}
	context.delete();
    }
	...
	synchronized (global_cache) {
		...
		if (context != null) classcache.remove(id);
	    }
	}
    ...
    protected void addChild(Object id, ResourceContext child) {
	 ...
	 if (preferredParent == null) {
	     ...
	     synchronized (global_cache) {
		 HashMap classcache = (HashMap) 
		     global_cache.get(child.getContextKind());
	...
	try {
		...
		java.lang.reflect.Constructor cons = 
		    cl.getConstructor(constructorParamTypes);
		...
		if (logger.isInfoEnabled())
		    logger.info("Created DataFeed '" + feed.getName() +
				"' of " + rawfeed.getClass());
	    }
	} catch (Exception ex) {
	    logger.error(null, ex);
