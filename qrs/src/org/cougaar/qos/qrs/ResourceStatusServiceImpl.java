...
public class ResourceStatusServiceImpl 
    implements ResourceStatusServiceOperations
{
    ...
    private TimerQueueingDataFeed corbaFeed;
	...
	if (debugLogger.isDebugEnabled()) debugLogger.debug("postInit()");
	...
	if (name != null) CorbaUtils.nsBind(name, servant);
    }

    public void pushString(String key, String value)
    {
	if (eventLogger.isInfoEnabled())
	    eventLogger.info("Method entry: pushString " 
			     +key+ "->" + value);
	corbaFeed.newData(key, new DataValue(value), null);
	if (eventLogger.isInfoEnabled())
	    eventLogger.info("Method exit: pushString " 
			     +key+ "->" + value);
    }
    ...
    public boolean invoke_s(String path_string, 
			    String method,
			    String[] args)
    ...
    public boolean invoke(ResourceNode[] path, 
			  String method,
			  String[] args)
	throws org.cougaar.qos.ResourceStatus.NoSuchMethodException
    {
...
// 	RSS.instance().registerFeed(relay_feed, "Relay");
	...
	ResourceContext context =
	    RSS.instance().resolveSpec(node.kind, node.parameters);
	if (context != null) {
	...
	FormulaObserver(DataValueHolder result, 
			NotificationQualifier qualifier)
	{
	    this.result = result;
    ...
    // credibility.
    private static final NotificationQualifier Not_No_Value =
	new NotificationQualifier() {
	    public boolean shouldNotify(DataValue value) {
		return value.getCredibility() > 0.0;
	    }
	};

    public boolean blockingQuery_s(String path_string,
				   long timeout,
				   DataValueHolder result)
    ...
    public boolean blockingQuery(ResourceNode[] path, 
				 long timeout,
				 DataValueHolder result)
    {
	boolean success = false;
	...
	if (qualifier != null) {
	    try {
		nqualifier = (NotificationQualifier)
		    CorbaUtils.poa.reference_to_servant(qualifier);
	    } catch (Exception ex) {
	...
	try {
	    ...
	    RSSSubscriberProxy proxy = 
		new RSSSubscriberProxy(bdf, subscriber, callback_id, this);
    ...
    public void unsubscribe_s(RSSSubscriber subscriber, 
			      String path_string) 
	throws ResourceDescriptionParseException
    {
	ResourceNode[] path = PathParser.parsePath(path_string);
    ...
    public void unsubscribe(RSSSubscriber subscriber, 
			    ResourceNode[] path) 
    {
	String logmsg = null;
    ...
    private static class EveryQualifier
	extends QualifierPOA
	implements NotificationQualifier
    {
	...
	public boolean shouldNotify(DataValue value)
	{
		...
		NotificationQualifier qualifier = (NotificationQualifier)
		    qualifiers[i];
		if (!qualifier.shouldNotify(value)) return false;
	    }
	    return true;
	}

	public void getAttribute(String attr_name,
				 data_valueHolder attr_value) 
	    throws NoSuchAttributeException
	{
	    throw new NoSuchAttributeException(attr_name);
	}

	public void setAttribute(String attr_name,
    ...
    private static class SomeQualifier
	extends QualifierPOA
	implements NotificationQualifier
    {
	Qualifier[] qualifiers;
	...
	public boolean shouldNotify(DataValue value)
	{
		...
		NotificationQualifier qualifier = (NotificationQualifier)
		    qualifiers[i];
		if (qualifier.shouldNotify(value)) return true;
	    }
	    return false;
	}
    ...
    private static abstract class ThresholdQualifierPOA
	extends QualifierPOA
	implements NotificationQualifier
    {
	double threshold;
    ...
    private static class MinDeltaQualifier 
	extends ThresholdQualifierPOA
    {
	DataValue last;
    ...
    private static class MinCredibilityQualifier 
	extends ThresholdQualifierPOA
    {
    ...
    private static class ExceedsThresholdQualifier 
	extends ThresholdQualifierPOA
    {
    ...
    private static class CrossesThresholdQualifier 
	extends ThresholdQualifierPOA
    {
	DataValue last;
    ...
    private static class EveryQualifierFactoryImpl
	extends EveryQualifierFactoryPOA
    {
    ...
    private static class SomeQualifierFactoryImpl
	extends SomeQualifierFactoryPOA
    {
	public Qualifier getQualifier(Qualifier[] qualifiers) {
    ...
    private static class MinDeltaQualifierFactoryImpl
	extends MinDeltaQualifierFactoryPOA
    {
	...
	extends ExceedsThresholdQualifierFactoryPOA
    {
	public Qualifier getQualifier(double threshold) {
	...
	extends CrossesThresholdQualifierFactoryPOA
    {
	public Qualifier getQualifier(double threshold) {
	...
	extends MinCredibilityQualifierFactoryPOA
    {
	public Qualifier getQualifier(double threshold) {
