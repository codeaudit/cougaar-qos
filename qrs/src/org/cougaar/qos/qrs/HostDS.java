...
/**
 * A sample ResourceContext which looks for Host capacity data on any
 * feed, by using a Remos-style key with an IntegraterDS.  The
...
public class HostDS extends ResourceContext implements Constants
{
	...
	ContextInstantiater cinst = new AbstractContextInstantiater() {
		...
		public Object identifyParameters(String[] parameters) 
		{
		    ...
		    String result;
		    try {
			result = InetAddress.getByName((String) x).getHostAddress();
	...
	int positiveB =  (b < 0) ? 256+b : (int) b;
	return Integer.toString(positiveB);
    }
    ...
    /**
     * The parameters should contain one string, the address of the
    ...
    private HostDS(String[] parameters, ResourceContext parent) 
	throws ParameterError
    {
	...
	private static final DataValue defaultValue = 
	    new DataValue(false, DEFAULT_CREDIBILITY);
	...
	protected void initialize(ResourceContext context) {
	    ...
	    // Canonicalize ipAddr here
	    String key = "Ip" + KEY_SEPR + 
		"Flow" + KEY_SEPR + 
		localIpAddr + KEY_SEPR + 
		remoteIpAddr + KEY_SEPR + 
		// JAZ TOS bits hard wired to 1 for now
		"1"+ KEY_SEPR + 
		"PathCost-Hops";
	    String[] parameters = { key };
	    ResourceContext hops = 
		RSS.instance().resolveSpec("Integrater", parameters);
